...
CREATE PROCEDURE [dbo].[p_classificator_insert] 
...
BEGIN TRY
	...
	BEGIN
		...
		SET @IdParent = ISNULL(@IdParent,0)
		...
		/*SET @Sql = 'IdObject='+CAST( @IdObject as VARCHAR )

	EXEC dbo.p_event 
		@IdMessage=0, 
		@IdUser=0, 
		@UserLogin='', 
		@TextEvent='Проверка ТЕСТ [dbo].[p_classificator_insert] ', 
		@TextNote=@Sql*/



		IF EXISTS(SELECT * FROM A.ENTITY_LOCKED WHERE CodeEntity=@EntityName AND IdObject < 0)
			EXEC dbo.p_error 'Запрещено добавлять данные к закрытому классификатору', 102011
		...
		IF EXISTS( SELECT * FROM ' + @TableName + ' WHERE IdObject = (SELECT IdObject FROM @Table1 ) )			
			UPDATE @Table1 SET IdObject = (SELECT 1 + IsNull(MAX(IdObject),0) FROM ' + @TableName + ' ) 
		...
		IF ('+CAST(@IdParent as VARCHAR)+'>0) AND ('+CAST(@RankObject as VARCHAR)+'=0) 
		UPDATE @Table1 SET RankObject=(SELECT 1+RankObject FROM ' + @TableName + ' WHERE IdObject='+CAST(@IdParent as VARCHAR)+')
...
END TRY BEGIN CATCH 
	...
	SET @ReturnNote = 'SP [dbo].[p_classificator_insert] error: Entity='+@EntityName + ';IdObject=' + CAST(@IdObject as VARCHAR) + ';NameObject='+@NameObject 
